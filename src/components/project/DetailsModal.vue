...
<template>
    <div v-if="show && details" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" role="dialog" aria-modal="true">
        <div class="bg-white rounded-lg p-6 mx-3 md:mx-0 md:max-w-[80%] relative max-h-[90vh] overflow-y-auto">
            <button class="fixed top-5 right-5 text-gray-600 bg-white hover:text-gray-900 hover:bg-gray-200 border rounded-md p-1" @click="emit('close')" aria-label="Fechar Modal">
            ...
            <ul class="grid md:grid-cols-2 mt-4 gap-4">
                <li v-for="(detail, index) in details.imageDetails" :key="index" class="ring-1 ring-gray-300 rounded-md overflow-hidden">
                    <img
