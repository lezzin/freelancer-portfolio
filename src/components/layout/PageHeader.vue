...
<template>
  <header
    ...
    :class="
      hasHeaderBackground
        ? 'ring-slate-300 bg-white bg-opacity-80 backdrop-blur-md'
        : 'ring-green-900 bg-green-800'
    "
  >
    <PageContainer
      class="flex items-center justify-between h-[10vh]"
      aria-label="Global"
    >
      <AppLogo />
      ...
      >
        ...
        <button
          ...
          :class="`-m-2.5 rounded-md p-2.5 ${
            hasHeaderBackground ? 'text-black' : 'text-white'
          }  hover:text-red-600`"
          @click="toggleMenu"
        >
